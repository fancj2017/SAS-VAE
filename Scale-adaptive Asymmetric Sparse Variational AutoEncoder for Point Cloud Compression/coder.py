...
class CoordinateCoder():
    ...
    def encode(self, coords, postfix=''):
        coords = coords.numpy().astype('float')#int
        coords = coords*xn
...
class Coder():
    ...
    def decode(self, rho=1, postfix=''):
        ...
        y_C = torch.cat((torch.zeros((len(y_C), 1)).float(), torch.tensor(y_C).float()), dim=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckptdir", default='ckpts/tp/epoch_49.pth')
    ...
    # po2po error
    start_time = time.time()
    pc_error_metrics = pc_error(p+'.ply', filename+'_dec.ply', res=args.res, show=False)
