...
import random
import os, time
from data_utils import read_ply_ascii_geo, write_ply_ascii_geo, write_h5_geo, read_h5_geo
...
def mesh2pc(mesh_filedir):
    points = np.asarray(read_h5_geo(mesh_filedir))
    # random rotate.
...
def generate_dataset(mesh_filedirs, pc_rootdir, out_filetype):
    start_time = time.time()
    for idx, mesh_filedir in enumerate(mesh_filedirs):
        try:
            points = mesh2pc(mesh_filedir)
        except:
...
if __name__ == "__main__":
    ...
    mesh_filedirs = [f for f in input_filedirs if
                     (os.path.splitext(f)[1] == '.ply' or os.path.splitext(f)[1] == '.h5')]  # .off or .obj

    print("mesh_filedirs:\n", len(input_filedirs), len(mesh_filedirs))
    if not os.path.exists(pc_rootdir): os.makedirs(pc_rootdir)

    generate_dataset(mesh_filedirs, pc_rootdir, out_filetype)
