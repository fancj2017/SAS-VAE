...
def read_h5_geo(filedir):
    ...
    coords = pc[:,0:3].astype('float')#coords = pc[:,0:3].astype('int')
...
def write_ply_ascii_geo(filedir, coords):
    ...
    f.write('end_header\n')
    coords = coords.astype('float')#coords = coords.astype('int')
    coords = coords/xn
    for p in coords:
...
def load_sparse_tensor(filedir, device):
    coords = torch.tensor(read_ply_ascii_geo(filedir)).float()#coords = torch.tensor(read_ply_ascii_geo(filedir)).int()
    #coords = (coords * 16).float()
    feats = torch.ones((len(coords),1)).float()
    coords, feats = ME.utils.sparse_collate([coords], [feats], dtype=torch.float32)
    x = ME.SparseTensor(features=feats, coordinates=coords, tensor_stride=1, device=device)
    
    return x

def scale_sparse_tensor(x, factor):
    coords = (x.C[:, 1:] * factor).float()
